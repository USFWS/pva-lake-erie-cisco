        ...
        overflow: scroll;
            ...
            tabItem(tabName = "menuSurvival",
                            ...
                            fluidRow(box(title="Age-1+ Survival",status="primary",solidHeader=TRUE,width=9,
                                         ...
                                         "This is accomplished by randomly selecting an error (+/- 30% for age-1 fish; +/-10% for fish age-2 and older) and adding that value to the entire vector of survival at age for the given year.",br(),
                                         "When stocked fish first enter the model, an age-invariant immediate stocking survival rate (finite) is applied.",br(), 
                                         "Following immediate stocking survival, and for the remainder of the model year, stocked individuals are subject to age-specific survival rates that are one half the rates computed for wild fish.")),
            ...
            tabItem(tabName = "menuDim",
                                ...
                                fluidRow(box(sliderInput("n.projections","Number projection years",value=50,min=0,max=200,step=5),width=9)),
                                fluidRow(box(sliderInput("n.simulations","Number simulations",value=100,min=0,max=10000,step=100),width=9))),
            ...
            tabItem(tabName = "menuStocking",
                                ...
                                fluidRow(box(numericInput("n.stockyrs","Number years to stock",value=10,min=1,max=50,step=1),width=4)),
                                fluidRow(box(numericInput("n.stockage","Number ages to stock",value=1,min=1,max=5,step=1),width=4)),
            ...
            tabItem(tabName = "menuAdvanced",
                    ...
                    fluidRow(column(4,style="width:25%;",
                                    box(title="Egg Survival (finite)",status="primary",solidHeader=TRUE,width=NULL,
                                        radioButtons("adv.eggS.sel","Egg survival options",opts.adv),
                                        numericInput("eggS.mu.custom","Mean (default = 0.00863)",0.00863,min=0,max=0.5,step=0.0001),
                                        numericInput("eggS.var.custom","Variance (default = 0.000163)",0.000163,min=0,max=0.5,step=0.000001))),
                             column(4,style="width:25%",
                                    box(title="Fry Survival (finite)",status="primary",solidHeader=TRUE,width=NULL,
                             ...
                             column(4,style="width:25%",
                                    box(title="Age-0 Survival (finite)",status="primary",solidHeader=TRUE,width=NULL,
                    ...
                    fluidRow(box(title="Custom Age-1+ Natural Mortality (instantaneous)",status="primary",solidHeader=TRUE,width=9,rHandsontableOutput("hot")))
